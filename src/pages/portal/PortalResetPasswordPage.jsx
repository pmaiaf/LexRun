...
export default function PortalResetPasswordPage() {
  ...
  async function handleSubmit(e) {
    ...
    setLoading(true)
    try {
      await portalService.redefinirSenha(token, novaSenha)
      setSucesso(true)
    } catch (err) {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-white p-8">
        <div className="w-full max-w-sm text-center">
          ...
          <button onClick={() => navigate('/portal/esqueci-senha')} className="btn-primary w-full py-2.5">Solicitar novo link</button>
        </div>
      </div>
    )
  }
      ...
      <div className="w-full max-w-sm">
        {sucesso ? (
          <div className="text-center">
            ...
            <button onClick={() => navigate('/portal/login')} className="btn-primary w-full py-2.5">Ir para o login</button>
          </div>
        ) : (
          <>
            <div className="mb-7">
